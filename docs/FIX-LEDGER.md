...
| `WaySystemServer.startServices()` 按资源数组 `config_externalWayDroidServices` 里的字符串启动 `id.waydro.server.WayDroidService`，R8 看不到资源引用，把类从优化后的 services.jar 剥离，system_server 启动时报 `service class not found`，Waydroid 框架服务（platform/notifications/hardware/usermonitor）全部缺失 | `services/proguard.flags` 对 `id.waydro.**` 加 keep 规则（window series 0018） | 已纳入 window series；真机部署后 `service list` 可见 `waydroidplatform` 等四个 binder 服务注册，`service class not found` 归零 |
